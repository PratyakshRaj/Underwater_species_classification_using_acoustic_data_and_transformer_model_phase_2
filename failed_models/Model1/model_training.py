...
class trainer:
    def __init__(self,path_spec_folder,path_mfcc_folder,label_map_path,d_mfcc_features, d_model, num_classes,n_heads,n_layers, fold,train_subset=None, val_subset=None):
        ...
        #self.train_loader = DataLoader(self.train_dataset, batch_size=8, shuffle=True,collate_fn=collate_fn)

        self.device = "cuda" if torch.cuda.is_available() else "cpu"
        # d_mfcc_features=36, d_model=512, num_classes=25
        self.model = TransformerModel(input_dim=d_mfcc_features,d_model=d_model,num_classes=num_classes,n_heads=n_heads,n_layers=n_layers).to(self.device)
        self.fold=fold
    ...
    def train(self,epochs):
        for epoch in range(epochs):
            ...
            for batch_idx,batch in enumerate(self.train_loader):
                ...
                outputs = self.model(mfcc,mfcc_mask)
